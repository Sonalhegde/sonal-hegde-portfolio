...
- Marine debris detection internship project at Sultan Qaboos University, Muscat, Oman. Built a YOLOv8 pipeline for drone imagery, curated and annotated a floating-debris dataset, exceeded 90% detection accuracy, and integrated a live web dashboard.
...
export function portfolioFallbackAnswer(question: string) {
  ...
  if (/marine|yolo|debris|oman/.test(query)) return "Sonal built a YOLOv8 marine-debris detection pipeline during an internship project at Sultan Qaboos University in Oman. It used drone imagery, a custom annotated floating-debris dataset, exceeded 90% detection accuracy, and fed a live web dashboard.";
