...
export function portfolioFallbackAnswer(question: string) {
  ...
  if (/contact|email|reach|hire/.test(query)) return "You can reach Sonal at sonalhhegde@gmail.com. The Contact section also includes mail, Gmail, GitHub, LinkedIn, and résumé links.";
  if (/marine|yolo|debris|oman|jetson/.test(query)) return "Sonal built a YOLOv8 marine-debris detection pipeline during an internship project at Sultan Qaboos University in Oman. It used drone imagery, a custom annotated floating-debris dataset, exceeded 90% detection accuracy, was optimized for Jetson Nano, and fed a live web dashboard. The résumé does not list the project dates.";
  ...
  if (/cert/.test(query)) return "The résumé lists certifications from Atlassian, McKinsey.org, Palo Alto Networks, Anthropic, NVIDIA, AWS, Cisco, HP LIFE, and Infosys. Issue dates and credential links are not present in the résumé.";
  ...
  return "I can answer questions about Sonal’s experience, projects, skills, education, certifications, résumé, or contact details. Try asking about the NITK work, marine-debris project, or embedded stack.";
}
